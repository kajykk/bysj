...
|------|------|----------|----------|----------|----------|
| P3 低 | 31 | ISS-108 ~ ISS-138 | 后端代码质量、前端代码质量、视觉细节、响应式细节、UX 细节、A11Y 细节 | 低优先级，不影响功能与安全；本轮聚焦 P0/P1/P2 修复与 Phase 6 验收闭环 | ✅ 已全部关闭（2026-08-08 v1.40-followup，见 05/07 文档）|
| P4 建议 | 12 | ISS-139 ~ ISS-150 | 代码质量、命名规范、维护性建议 | 建议级，不影响交付；属代码风格与可维护性优化 | ✅ 已收口（2026-08-08：sanitized 命名误导修复；其余为架构约束（TRAINING_JOBS/_inflight_futures 跨进程不共享，已有 Redis 同步/单机收敛）或有测试覆盖的有意保留模块（middleware/xss.py））|

**横向排查结论**：P3/P4 43 项均为独立问题，无同类横向扩散风险；已确认无 P3/P4 问题阻塞核心功能链路或引入安全风险。

**状态说明**：截至 2026-08-08，P3 31 项 + P4 12 项（ISS-139~150 组）已全部收口关闭，v1.40 审计 164 项全部闭环。
