...
sample_json = ROOT / "infra/grafana/dashboards/v1.37-alerts-overview.json"
...
try:
    ...
    if missing:
        ...
        sys.exit(1)
    print("  PASS: dashboard JSON schema valid (Grafana 11.6)")
...
try:
    ...
    for ds in cfg.get("datasources", []):
        print(
            f"  datasource: {ds.get('name')} (type={ds.get('type')}, url={ds.get('url')})"
        )
        assert ds.get("type") == "simpod-json-datasource", "wrong datasource type"
        assert ds.get("url"), "datasource url missing"
        assert ds.get("secureJsonData", {}).get(
            "Authorization"
        ), "Authorization header missing"
    print("  PASS: datasource provisioning YAML valid")
except Exception as e:
    print(f"  FAIL: {e}")
    sys.exit(1)
...
dp_yaml = ROOT / "infra/grafana/provisioning/dashboards/alerts-overview.yaml"
try:
    ...
    for p in cfg.get("providers", []):
        print(
            f"  provider: {p.get('name')} folder={p.get('folder')} path={p.get('options', {}).get('path')}"
        )
        assert p.get("options", {}).get("path"), "provider path missing"
    print("  PASS: dashboard provider provisioning YAML valid")
except Exception as e:
    print(f"  FAIL: {e}")
    sys.exit(1)
...
try:
    ...
    print(
        f"  GF_PLUGINS_PREINSTALL: {svc.get('environment', {}).get('GF_PLUGINS_PREINSTALL')}"
    )
    assert "simpod-json-datasource" in (
        svc.get("environment", {}).get("GF_PLUGINS_PREINSTALL", "") or ""
    )
    print("  PASS: docker-compose grafana service valid")
except Exception as e:
    print(f"  FAIL: {e}")
    sys.exit(1)
