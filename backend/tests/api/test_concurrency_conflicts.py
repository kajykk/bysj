...
def test_task_state_conflict_on_double_complete(
...
def test_warning_handle_conflict_by_other_counselor(
    ...
    res = client.put(
        f"/api/v1/counselor/warnings/{warning_id}/handle", json={"action": "handle"}
    )
    assert res.status_code == 404


def test_add_group_member_idempotent_under_repeated_calls(
    client: TestClient, as_role, seed_counselor_data: None
) -> None:
    as_role("counselor", 2)
    group_res = client.post(
        "/api/v1/counselor/groups", json={"group_name": "G1", "color_tag": "#409EFF"}
    )
