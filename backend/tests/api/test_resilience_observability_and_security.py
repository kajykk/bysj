...
def test_assessment_submit_writes_risk_log(client: TestClient, as_role, seeded_user_id: int) -> None:
...
def test_permission_boundary_user_cannot_access_counselor_api(client: TestClient, as_role) -> None:
    as_role('user', 1)
    res = client.get('/api/v1/counselor/warnings')
    assert res.status_code == 403


def test_permission_boundary_counselor_cannot_access_admin_api(client: TestClient, as_role) -> None:
    as_role('counselor', 2)
