...
class TestStructuredExpectedRisk:
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize("test_case", STRUCTURED_TEST_CASES, ids=lambda x: x["name"])
    async def test_structured_risk_levels(self, model_engine: ModelEngine, test_case: dict) -> None:
        """验证结构化模型风险等级在预期范围内。"""
