...
class TestPredictPhysiologicalConfidence:
    ...
    async def test_partial_data_confidence(self, engine: ModelEngine) -> None:
        ...
        physiological = {
            "sleep_hours": 8, "sleep_quality": 7, "exercise_minutes": 30,
            "heart_rate": 75, "steps": 8000,  # 缺失 systolic_bp, diastolic_bp
        }
        result = await engine.predict_physiological(physiological)
    ...
    async def test_poor_data_confidence(self, engine: ModelEngine) -> None:
        """缺失 3+ 个字段应返回低置信度。"""
        physiological = {
            "sleep_hours": 8, "steps": 8000,  # 缺失 5 个字段
        }
        result = await engine.predict_physiological(physiological)
    ...
    async def test_threshold_calibration(self, engine: ModelEngine) -> None:
        ...
        physiological = {
            "sleep_hours": 3, "sleep_quality": 2, "exercise_minutes": 0,
            "heart_rate": 110, "systolic_bp": 150, "diastolic_bp": 95, "steps": 1000,
        }
        result = await engine.predict_physiological(physiological)
