...
class TestPredictFusionPriority:
    ...
    async def test_crisis_override_in_fusion(self, engine: ModelEngine) -> None:
        ...
        result = await engine.predict_fusion(
            features={"age": 20, "gender": 1, "stress_level": 1, "sleep_duration": 8, "social_support": 5, "academic_pressure": 1, "anxiety": 1, "panic_attack": 0, "treatment_seeking": 0},
            text="我不想活了，想结束这一切。",
        )
    ...
    async def test_review_required_single_high(self, engine: ModelEngine) -> None:
        """单模态高风险应标记复核。"""
        result = await engine.predict_fusion(
            features={"age": 20, "gender": 1, "stress_level": 5, "sleep_duration": 3, "social_support": 1, "academic_pressure": 5, "anxiety": 5, "panic_attack": 1, "treatment_seeking": 0},
            text="最近有点累，但总体还好。",
        )
