...
class TestNullAndMissingFieldsAPI:
    ...
    def test_validation_run_missing_model_version(self, client: TestClient, as_role) -> None:
    ...
    def test_validation_run_missing_dataset_path(self, client: TestClient, as_role) -> None:
    ...
    def test_validation_run_null_model_version(self, client: TestClient, as_role) -> None:
    ...
    def test_validation_run_null_dataset_path(self, client: TestClient, as_role) -> None:
    ...
    def test_monitoring_success_rate_invalid_granularity(self, client: TestClient, as_role) -> None:
        """验证无效 granularity 参数不触发 500"""
        as_role("admin", 3)
        response = client.get("/api/v1/monitoring/model-success-rate?granularity=invalid")
    ...
    def test_input_validator_none_input(self) -> None:
        ...
        assert any(e["anomaly_type"] in ("type_error", "null_input") for e in result.errors)
    ...
    def test_input_validator_empty_dict(self) -> None:
        ...
        assert any(e["anomaly_type"] in ("missing_required", "empty_input", "type_error") for e in result.errors)
    ...
    def test_all_endpoints_no_500_on_null_input(self, client: TestClient, as_role) -> None:
