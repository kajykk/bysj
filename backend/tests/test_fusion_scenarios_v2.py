...
class TestFusionScenariosV2:
    ...
    def setup_method(self) -> None:
        ...
        project_root = Path(__file__).resolve().parents[2]
        self.scenarios_path = (
            project_root / "datasets" / "fusion" / "fusion_test_scenarios_v2.json"
        )
    ...
    def test_scenario_count(self) -> None:
        """验证场景数量."""
        assert (
            len(self.scenarios) >= 49
        ), f"Expected at least 49 scenarios, got {len(self.scenarios)}"
    ...
    def test_single_modality_scenarios(self) -> None:
        ...
        for scenario_name in single_modality:
            scenario = next(
                (s for s in self.scenarios if s["scenario"] == scenario_name), None
            )
            assert scenario is not None, f"Missing scenario: {scenario_name}"

            payload = scenario["payload"]
            modalities = [
                k for k in payload.keys() if k in ["features", "text", "physiological"]
            ]
            assert (
                len(modalities) == 1
            ), f"{scenario_name} should have exactly 1 modality, got {len(modalities)}"

    def test_dual_modality_scenarios(self) -> None:
        """验证双模态场景."""
        dual_modality = [
            "structured_text_low_low",
            "structured_text_low_high",
            "structured_text_high_low",
            "structured_text_high_high",
            "structured_physio_low_low",
            "structured_physio_low_high",
            "structured_physio_high_low",
            "structured_physio_high_high",
            "text_physio_low_low",
            "text_physio_low_high",
            "text_physio_high_low",
            "text_physio_high_high",
        ]
        ...
        for scenario_name in dual_modality:
            ...
            assert (
                len(modalities) == 2
            ), f"{scenario_name} should have exactly 2 modalities, got {len(modalities)}"

    def test_triple_modality_scenarios(self) -> None:
        """验证三模态场景."""
        triple_modality = [
            "all_modalities_low_low_low",
            "all_modalities_high_high_high",
            "all_modalities_mixed_1",
            "all_modalities_mixed_2",
            "all_modalities_mixed_3",
        ]
        ...
        for scenario_name in triple_modality:
            ...
            assert (
                len(modalities) == 3
            ), f"{scenario_name} should have exactly 3 modalities, got {len(modalities)}"
    ...
    def test_boundary_scenarios(self) -> None:
        ...
        boundary_scenarios = [
            ...
            "boundary_sleep_min",
            "boundary_sleep_max",
            "boundary_heart_rate_min",
            "boundary_heart_rate_max",
            "boundary_steps_min",
            "boundary_steps_max",
            "boundary_bp_min",
            "boundary_bp_max",
        ...
        for scenario in self.scenarios:
            ...
            if "features" in payload:
                ...
                for feat in required_features:
                    assert (
                        feat in features
                    ), f"Missing feature {feat} in {scenario['scenario']}"
            ...
            if "physiological" in payload:
                ...
                for p in required_physio:
                    assert (
                        p in physio
                    ), f"Missing physiological {p} in {scenario['scenario']}"
    ...
    def test_high_risk_scenarios(self) -> None:
        ...
        for scenario_name in high_risk_scenarios:
            ...
            assert (
                scenario["label"] == 1
            ), f"{scenario_name} should be high risk (label=1)"
    ...
    def test_low_risk_scenarios(self) -> None:
        ...
        for scenario_name in low_risk_scenarios:
            ...
            assert (
                scenario["label"] == 0
            ), f"{scenario_name} should be low risk (label=0)"
