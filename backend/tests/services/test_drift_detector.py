"""T-COV: PsiKlCalculator (services) 单元测试.
...
@pytest.fixture
def service(db_session: AsyncSession) -> PsiKlCalculator:
...
class TestCalculatePsi:
    ...
    def test_psi_identical_distributions_low(self, service: PsiKlCalculator):
    ...
    def test_psi_shifted_distribution_high(self, service: PsiKlCalculator):
    ...
    def test_psi_empty_baseline_returns_zero(self, service: PsiKlCalculator):
        """TC-COV-DRIFT-003: 空 baseline 返回 0.0 (早退分支 L30-31)."""
        psi = service.calculate_psi([], [1.0, 2.0, 3.0])
        assert psi == 0.0

    def test_psi_empty_current_returns_zero(self, service: PsiKlCalculator):
        """TC-COV-DRIFT-004: 空 current 返回 0.0 (早退分支 L30-31)."""
        psi = service.calculate_psi([1.0, 2.0, 3.0], [])
        assert psi == 0.0

    def test_psi_constant_distribution_returns_zero(self, service: PsiKlCalculator):
        """TC-COV-DRIFT-005: min==max 退化情形返回 0.0 (早退分支 L36-37)."""
        psi = service.calculate_psi([5.0, 5.0, 5.0], [5.0, 5.0, 5.0])
        assert psi == 0.0

    def test_psi_custom_buckets_min_clamped(self, service: PsiKlCalculator):
    ...
    def test_psi_accepts_iterable_inputs(
        self, service: PsiKlCalculator, seeded_user_id: int
    ):
    ...
    def test_psi_non_finite_returns_zero(self, service: PsiKlCalculator, monkeypatch):
...
class TestCleanArray:
    ...
    def test_clean_array_handles_type_error(self, service: PsiKlCalculator):
