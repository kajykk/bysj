...
from app.models.admin import ContentViewHistory, EducationContent, MeditationLog, UserFavorite
...
class ContentService:
    ...
    async def list_contents(
        ...
        count_stmt = select(func.count()).select_from(EducationContent).where(EducationContent.status == "active")
        ...
        stmt = stmt.order_by(EducationContent.sort_order.asc(), EducationContent.created_at.desc()).offset(offset).limit(page_size)
        rows = (await self.db.execute(stmt)).scalars().all()
        total = (await self.db.execute(count_stmt)).scalar_one()

        favor_stmt = select(UserFavorite.content_id).where(UserFavorite.user_id == user_id)
    ...
    async def get_content_detail(self, user_id: int, content_id: int) -> dict | None:
        ...
        # 同步内存对象，确保返回值反映最新计数（近似值，用于展示）
        content.view_count = (content.view_count or 0) + 1
        self.db.add(ContentViewHistory(user_id=user_id, content_id=content_id))

        favorite_stmt = select(UserFavorite).where(UserFavorite.user_id == user_id, UserFavorite.content_id == content_id)
    ...
    async def toggle_favorite(self, user_id: int, content_id: int) -> bool:
        ...
        stmt = select(UserFavorite).where(UserFavorite.user_id == user_id, UserFavorite.content_id == content_id)
    ...
    async def log_meditation(self, user_id: int, content_id: int | None, completed: bool) -> int:
    ...
    async def get_recommendations(self, user_id: int, page: int, page_size: int) -> dict:
        risk_stmt = (
        ...
        risk_level = latest_risk.risk_level if latest_risk else 1

        preferred_categories_stmt = (
            select(EducationContent.category, func.count(ContentViewHistory.id).label("cnt"))
            .join(ContentViewHistory, ContentViewHistory.content_id == EducationContent.id)
            .where(ContentViewHistory.user_id == user_id, EducationContent.status == "active")
            .group_by(EducationContent.category)
            .order_by(func.count(ContentViewHistory.id).desc())
            .limit(3)
        )
        preferred_categories = [x[0] for x in (await self.db.execute(preferred_categories_stmt)).all() if x[0]]
        ...
        ranked_stmt = (
            base_stmt.order_by(priority_case, EducationContent.view_count.desc(), EducationContent.created_at.desc())
            .offset(offset)
            .limit(page_size)
        )
        ...
        if not rows and page > 1 and total > 0:
            rows = (await self.db.execute(base_stmt.order_by(EducationContent.view_count.desc(), EducationContent.created_at.desc()))).scalars().all()
            rows = rows[offset: offset + page_size]

        favorite_ids = set((await self.db.execute(select(UserFavorite.content_id).where(UserFavorite.user_id == user_id))).scalars().all())
            ...
            "items": [
                {
                    ...
                    "view_count": row.view_count,
                    "is_favorited": row.id in favorite_ids,
                    "recommend_reason": self._recommend_reason(row.category or "", preferred_categories, risk_category),
                }
                for row in rows
            ],
            "total": total,
    ...
    async def list_recent_views(self, user_id: int, page: int, page_size: int) -> dict:
        ...
        stmt = (
            ...
            .join(latest_view_subq, latest_view_subq.c.content_id == EducationContent.id)
    ...
    @staticmethod
    def _recommend_reason(category: str, preferred_categories: list[str], risk_category: str) -> str:
