...
class ExperimentService:
    ...
    def import_dataset(
        ...
        result = self.data.import_dataset(
    ...
    def train_model(
        ...
        if model_name.startswith("physiological"):
            artifacts = self.trainer.train_physiological_model(
                train_df, val_df, model_name
            )
            return {
        ...
        artifacts = self.trainer.train(
            train_df, val_df, model_name, epochs, batch_size, learning_rate
        )
    ...
    def evaluate_model(
        ...
        if model_name.startswith("physiological"):
            ...
            df = pd.read_csv(
                self.data.test_path(dataset_name)
                if split == "test"
                else self.data.val_path(dataset_name)
            )
        return {
            "dataset_name": dataset_name,
            **self.evaluator.evaluate(df, model_name, split),
        }
