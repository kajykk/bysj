...
from app.models.user import User, UserProfile, EmergencyContact, UserCounselorBinding
...
class GDPRService:
    ...
    async def export_user_data(self, user_id: int) -> dict[str, Any]:
        ...
        bindings = (await self.db.execute(
            select(UserCounselorBinding).where(UserCounselorBinding.user_id == user_id)
        )).scalars().all()

        # 4. 风险评估
        risk_assessments = (await self.db.execute(
            select(RiskAssessment).where(RiskAssessment.user_id == user_id)
        )).scalars().all()

        # 5. 预警记录
        warnings = (await self.db.execute(
            select(WarningNotification).where(WarningNotification.user_id == user_id)
        )).scalars().all()

        # 6. 危机事件
        crisis_events = (await self.db.execute(
        ...
        if plan_ids:
            tasks = (await self.db.execute(
                select(InterventionTask).where(InterventionTask.plan_id.in_(plan_ids))
            )).scalars().all()

        # 8. 操作日志 (该用户的)
        op_logs = (await self.db.execute(
    ...
    async def anonymize_user(
        ...
        *,
        ...
        Args:
            user_id: 目标用户 ID
            password_confirm: 用户当前密码（用于二次确认）
        ...
        user.phone = None
        user.password_hash = "$2b$12$" + "0" * 53  # 有效 bcrypt 格式但永不匹配任何密码，防止 verify_password 异常
        user.status = "deleted"
        ...
        for s in sessions:
            s.revoked_at = dt.now(timezone.utc)
