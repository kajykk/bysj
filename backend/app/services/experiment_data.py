...
class ExperimentDataManager:
    ...
    def export_dataset_snapshot(self, dataset_name: str) -> dict[str, Any]:
        ...
        (DATA_ROOT / f"{dataset_name}.snapshot.json").write_text(json.dumps(snapshot, ensure_ascii=False, indent=2), encoding="utf-8")
        return snapshot
    ...
    def import_dataset(self, dataset_name: str, source_type: str, train_ratio: float, val_ratio: float, test_ratio: float) -> dict[str, Any]:
        ...
        if normalized_source_type in {"bert", "text", "nlp"} and "text" not in df.columns:
        ...
        train_df, temp_df = train_test_split(df, test_size=(1 - train_ratio), random_state=42, shuffle=True, stratify=stratify)
        ...
        val_df, test_df = train_test_split(temp_df, test_size=(1 - val_size), random_state=42, shuffle=True, stratify=temp_stratify)
        ...
        self.meta_path(dataset_name).write_text(json.dumps(meta, ensure_ascii=False, indent=2), encoding="utf-8")
        return {
            "dataset_name": dataset_name,
            "source_type": source_type,
            "total_samples": int(len(df)),
