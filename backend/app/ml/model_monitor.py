...
class ModelMonitor:
    ...
    def get_health_status(self) -> ModelHealth:
        ...
        error_rate = self.error_count / self.total_predictions if self.total_predictions > 0 else 0.0

        # Determine status
        alert_threshold = self.config.get("alert_threshold_consecutive_drifts", 3)

        if self.consecutive_drifts >= alert_threshold:
            status = "critical"
        elif self.consecutive_drifts > 0:
            status = "degraded"
        else:
            status = "healthy"
