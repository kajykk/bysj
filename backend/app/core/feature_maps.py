...
DEFAULTS: dict[str, Any] = {
    ...
    "AgeGroup": "19-25",
    # ── v1.23 外部 LR (Pipeline) 小写特征默认值 ──
    # v1.23 模型 feature_names_in_ 为小写原始列 (age/cgpa/...)，
    # _build_structured_input 对缺失特征用 _DEFAULTS.get(col, 0) 兜底；
    # 0 不在 v1.23 训练分布内（cgpa 训练范围 2.01-4.0 / sleep 4-9h /
    # stress 0-4 且系数 +0.90），实测缺失填 0 会使风险概率偏移 -51pt~+13pt。
    # 以下取 v1.23 训练集中位数 (scripts/modeling/v1_23/01_prepare_external_dataset.py
    # 产出的 train.csv)，与 Pipeline 内置 SimpleImputer(strategy="median") 语义一致。
    "age": 25.0,
    "gender": 1,
    "cgpa": 3.1,
    "stress_level": 2.4,
