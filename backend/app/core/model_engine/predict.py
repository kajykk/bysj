...
model_engine 包结构化拆分时整体迁入 `app.core.model_engine.predict`,
其中 `predict_fusion` 进一步拆分至 `app.core.model_engine.fusion`),
...
通过 Mixin 多继承模式装配到 ModelEngine:
    ...
    class ModelEngine(..., PredictMixin, ...):
        ...

依赖关系 (装配后由对应 Mixin / ModelEngine 主体提供):
- `self._load_model_async` / `self._load_adapter_async`  → LoadingMixin
- `self._timed_async` / `self._patch_simple_imputer`     → LoadingMixin / InferenceMixin
- `self._route_structured` / `self._update_structured_monitoring` → InferenceMixin
- `self._build_structured_input` / `self.feature_order`  → InferenceMixin / ModelEngine.__init__
- `self._incr_counter` / `self._incr_fallback`           → InferenceMixin
...
- `LiteFeatureExtractor`                                  → 延迟导入自 model_engine.inference
...
class PredictMixin:
    ...
    async def predict_structured(self, features: dict[str, float | int | str | bool]) -> dict[str, Any]:
        ...
        async with self._timed_async("predict", "structured"):
            ...
            default_model_version = getattr(settings, "structured_default_model", "v1.20")
            ...
            if model is not None and primary_needs_scaler:
                try:
                    structured_scaler = await self._load_model_async("structured_scaler_v1.20")
                except (FileNotFoundError, ValueError) as exc:
                    logger.warning(
            ...
            if model is not None:
                ...
                if model_feature_names:
                    ...
                    if structured_scaler is not None:
                        feature_scaled = await asyncio.to_thread(structured_scaler.transform, feature_df)
                    else:
                    ...
                    if structured_scaler is not None:
                        feature_scaled = await asyncio.to_thread(structured_scaler.transform, feature_array)
                    else:
            ...
            if settings.structured_experimental_enabled:
                ...
                logger.debug("experimental path skipped (structured_experimental_enabled=False)")
    ...
    async def _run_experimental_v121(self, raw: dict[str, Any], default_score: float) -> dict[str, Any]:
        ...
        if v1_21_info is not None and v1_21_info.lifecycle != "deprecated":
            try:
                ...
                try:
                    exp_scaled = await asyncio.to_thread(exp_scaler.transform, exp_array)
    ...
    async def _run_experimental_v123(self, raw: dict[str, Any], default_score: float) -> dict[str, Any]:
        ...
        delta = (
            round(experimental_external_score - default_score, 2) if experimental_external_score is not None else None
        )
    ...
    async def _run_adapter(
        ...
        try:
            ...
            if adapter is not None:
                ...
                adapter_input = v123_raw_score if v123_raw_score is not None else base_score
    ...
    async def _predict_text_ml(self, text: str) -> dict[str, Any]:
        ...
        try:
            ...
            logger.warning("Bilingual text model unavailable, falling back to primary: %s", exc)
            try:
    ...
    async def predict_lite(
        ...
        # LiteFeatureExtractor 定义在 model_engine.inference, 此处延迟导入保持原结构
        from app.core.model_engine.inference import LiteFeatureExtractor
        ...
        async with self._timed_async("predict", "lite"):
            ...
            if length < 20:
                logger.warning(
                    "Lite model: text too short (%d chars), " "falling back to anxiety_only",
                    length,
                )
                self._incr_fallback()
            ...
            feature_dict["coverage_density"] = kw["total_keywords"] / max(length, 1) * 100
            ...
            try:
                ...
                logger.warning("Lite model unavailable: %s, falling back to " "anxiety_only", exc)
                self._incr_fallback()
                return self._anxiety_only_fallback(gad7_score)
    ...
    async def _predict_physiological(self, data: dict[str, float | int]) -> float:
        ...
        try:
            ...
            logger.warning("Required dependency not installed, falling back to heuristic: %s", exc)
        except Exception as exc:
    ...
    def _predict_physiological_sync(self, data: dict[str, float | int]) -> float:
        ...
        if CLEANER_STATS_PATH.exists():
            try:
                ...
                logger.warning("Failed to load cleaner stats, using raw input: %s", clean_exc)
    ...
    async def explain_prediction(self, features: dict[str, float | int], model_id: str) -> list[dict[str, Any]]:
        ...
        if model_feature_names:
            ...
            try:
                factors = await asyncio.to_thread(self._compute_shap_factors, model, feature_df, model_feature_names)
                return factors
            except Exception as exc:
