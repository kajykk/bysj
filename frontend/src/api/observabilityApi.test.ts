...
describe('api/observabilityApi', () => {
  ...
  it('getHealth 调 /alerts/observability/health 并保留 envelope', async () => {
    (dedupedGet as any).mockResolvedValueOnce({ data: { data: { status: 'ok' }, instance_id: 'i1', cached: true, generated_at: 't' } })
    const res = await observabilityApi.getHealth()
    expect(dedupedGet).toHaveBeenCalledWith('/alerts/observability/health', { params: undefined })
  ...
  it('getTrend 传 bucket/severity/time range', async () => {
    (dedupedGet as any).mockResolvedValueOnce({ data: { data: { points: [] }, instance_id: 'i', cached: false, generated_at: 't' } })
    await observabilityApi.getTrend({ start_time: '2026-07-01', end_time: '2026-07-08', bucket: '6h', severity: 'high' })
    expect(dedupedGet).toHaveBeenCalledWith('/alerts/observability/trend', { params: { start_time: '2026-07-01', end_time: '2026-07-08', bucket: '6h', severity: 'high' } })
  })
  ...
  it('八个端点路径正确', async () => {
    ...
    for (const [fn, path] of endpoints) {
      (dedupedGet as any).mockResolvedValueOnce({ data: { data: {}, instance_id: 'i', cached: false, generated_at: 't' } })
      await (observabilityApi as any)[fn]()
      expect(dedupedGet).toHaveBeenCalledWith(path, { params: undefined })
    }
  })
