...
describe('Sentry Plugin - T-COV-SEN-FE-001', () => {
  ...
  describe('initSentry', () => {
    ...
    it('配置 DSN 后应调用 Sentry.init 并传入 dsn 与 environment', async () => {
    ...
    it('开发环境（PROD=false）默认 tracesSampleRate 应为 1.0', async () => {
      vi.stubEnv('VITE_SENTRY_DSN', 'https://test@sentry.io/1')
    ...
    it('生产环境（PROD=true）默认 tracesSampleRate 应为 0.01', async () => {
      vi.stubEnv('VITE_SENTRY_DSN', 'https://test@sentry.io/1')
    ...
    it('自定义 VITE_SENTRY_TRACES_SAMPLE_RATE 应覆盖默认值', async () => {
      vi.stubEnv('VITE_SENTRY_DSN', 'https://test@sentry.io/1')
    ...
    it('默认 replaysSessionSampleRate 应为 0.01，replaysOnErrorSampleRate 应为 1.0', async () => {
      vi.stubEnv('VITE_SENTRY_DSN', 'https://test@sentry.io/1')
    ...
    it('自定义 VITE_SENTRY_REPLAYS_SAMPLE_RATE 应覆盖默认值', async () => {
      vi.stubEnv('VITE_SENTRY_DSN', 'https://test@sentry.io/1')
    ...
    it('应正确配置 browserTracingIntegration 和 replayIntegration', async () => {
      vi.stubEnv('VITE_SENTRY_DSN', 'https://test@sentry.io/1')
      const mockRouter = createMockRouter()

      await initSentry(createMockApp(), mockRouter)
    ...
    it('应配置 ignoreErrors 过滤常见噪声错误', async () => {
      vi.stubEnv('VITE_SENTRY_DSN', 'https://test@sentry.io/1')
    ...
    it('应配置 denyUrls 过滤浏览器扩展 URL（4 个正则）', async () => {
      vi.stubEnv('VITE_SENTRY_DSN', 'https://test@sentry.io/1')
    ...
    it('VITE_APP_VERSION 配置时应传入 release', async () => {
      vi.stubEnv('VITE_SENTRY_DSN', 'https://test@sentry.io/1')
    ...
    it('VITE_APP_VERSION 未配置时不应传入 release', async () => {
      vi.stubEnv('VITE_SENTRY_DSN', 'https://test@sentry.io/1')
    ...
    it('environment 应取自 import.meta.env.MODE', async () => {
      vi.stubEnv('VITE_SENTRY_DSN', 'https://test@sentry.io/1')
